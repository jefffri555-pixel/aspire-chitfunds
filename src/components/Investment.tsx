...
export default function Investment() {
  ...
  return (
    <section id="investment" ref={sectionRef} className="section-padding bg-cream-pearl relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="mb-20 reveal">
          <div className="max-w-3xl mx-auto text-center mb-12">
            ...
            <p className="text-[#232323]/70 text-lg leading-relaxed">
          ...
          <div className="grid md:grid-cols-4 gap-4 mt-10">
            {[
              ...
              <div key={label} className="p-4 rounded-xl bg-white border border-[#CFAF5D]/15">
                ...
                <p className="text-[#232323]/60 text-xs">{val}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mb-20 reveal">
          <h3 className="font-serif text-3xl font-bold text-[#123524] text-center mb-12">Investment Strategies</h3>
            ...
            {strategies.map(({ name, risk, return: ret, desc }) => (
              <div key={name} className="p-6 rounded-2xl bg-white border border-[#CFAF5D]/15 premium-card">
                <div className="flex items-start justify-between mb-3">
                  <h4 className="font-serif text-xl font-bold text-[#123524]">{name}</h4>
                ...
                <p className="text-[#232323]/60 text-sm">{desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="reveal">
          <h3 className="font-serif text-3xl font-bold text-[#123524] text-center mb-12">Investment Options</h3>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 stagger mb-12">
            {investmentTypes.map(({ icon: Icon, title, desc }, i) => (
              <div key={title} className="reveal p-6 rounded-2xl bg-gradient-to-br from-white to-[#F7F2E8] border border-[#CFAF5D]/15 premium-card">
                <div className="w-12 h-12 rounded-xl bg-[#CFAF5D]/10 flex items-center justify-center mb-4">
                  <Icon className="text-[#CFAF5D]" size={24} />
                </div>
                <h4 className="font-serif text-lg font-bold text-[#123524] mb-2">{title}</h4>
                <p className="text-[#232323]/60 text-sm">{desc}</p>
              </div>
            ))}
          </div>
        ...
        {/* Benefits */}
        <div className="reveal bg-gradient-to-br from-[#FF0000] via-[#FF0000] to-[#FF0000] rounded-3xl px-8 py-12 text-center">
